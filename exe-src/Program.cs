...
app.MapGet("/files", (HttpRequest request) =>
{
    ...
    var downloadName = request.Query["download"].FirstOrDefault() == "1" ? name : null;
    return Results.File(file, contentType, fileDownloadName: downloadName, enableRangeProcessing: true);
});
